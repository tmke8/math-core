...
What you first need for font subsetting is the OTF file (not the WOFF2 file) which you can get from [math-core-fonts](https://github.com/tmke8/math-core-fonts).

The second thing you need is a text file containing all the Unicode characters that you want to use. The two following sections show how to generate such a text file in two different situations.

Once you have the file — let’s say it’s called `all_math_characters.txt` — you can use `hb-subset`, a command from the [HarfBuzz](https://harfbuzz.github.io/) project, to subset the font like this:

```sh
hb-subset \
	--output-file=NewCMMath-Book-subset.otf \
	--text-file=all_math_characters.txt \
	--layout-features=ssty,kern,aalt \
	--desubroutinize \
	NewCMMath-Book.otf
```
The flag `--layout-features=ssty,kern,aalt` is important here because it ensures we keep all the font features needed for math typesetting. The flag `--desubroutinize` removes any use of CFF subroutines, because it’s better to leave compression entirely to WOFF2. To get an even smaller file, one could remove the `ssty` font feature, because currently only Firefox makes use of it anyway, but this will obviously result in worse rendering in Firefox.

Alternatively, you can also use [fonttools](https://pypi.org/project/fonttools/) instead of `hb-subset` for font subsetting. The command to run will be similar.

Finally, compress the font with `woff2_compress` from the [woff2](https://github.com/google/woff2) project (I’ve tried other woff2 compressors, but they seemed to break the math table):

```sh
woff2_compress NewCMMath-Book-subset.otf
```

You can install `hb-subset` and `woff2_compress` on most Linux distributions from the default repositories. We don’t require an especially up-to-date version.

### Extracting required symbols for static site
If you are generating a static site, you can use the Python script `scripts/extract_math_chars.py` in this repository to generate the file with all the symbols:

```sh
python3 /path/to/math-core/scripts/extract_math_chars.py --skip-text -r -o all_math_characters.txt _site
```

where `--skip-text` is optional and tells the script to skip symbols within `<mtext>` for which you are encouraged to not use the math font (see the section “CSS for text-mode fonts” above). `-r` means the script recursively goes through the given directory and `-o` specifies the output file. `all_math_characters.txt` is then fed to `hb-subset` as explained above.

### General case
Even if you don’t know ahead-of-time which glyphs your math will use, we can still use font subsetting, because the set of symbols that `math-core` can ever output is smaller than the set of symbols defined in *New Computer Modern Math Book*.
...
The script writes its output to `scripts/all_symbols.txt`, which is the file you need to feed to `hb-subset`.

Running `hb-subset` with `--layout-features=ssty,kern,aalt`, as described above, should result in a WOFF2 file that’s about 450kB for *New Computer Modern Math Book*. If you remove the `ssty` font feature(which makes rendering worse on Firefox as mentioned above) the file is about 200kB.
