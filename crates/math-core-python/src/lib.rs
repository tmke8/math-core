...
impl LatexToMathML {
    #[classmethod]
    #[pyo3(signature = (*, pretty_print="never", macros=None, xml_namespace=false, continue_on_error=false))]
    fn with_config<'a>(
        _cls: &Bound<'_, PyType>,
        pretty_print: &str,
    ...
    fn convert_with_global_counter<'a>(
            ...
            .convert_with_global_counter(latex, display)
        {
            Err(mut latex_error) => {
                ...
                latex_error.0.0 = byte_offset_to_char_offset(latex, latex_error.0.0);
