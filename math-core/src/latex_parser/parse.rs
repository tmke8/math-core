    ...
    'source: 'arena, // The reference to the source string will live as long as the arena.
{
    ...
    fn parse_token(
        ...
        let node = match cur_token {
            ...
            Token::Error(err) => match err {
                TokenError::UnknownCommand(name) => {
                    return Err(LatexError(loc, LatexErrKind::UnknownCommand(name)));
                }
                TokenError::UnknownEnvironment(env_name) => {
                    return Err(LatexError(loc, LatexErrKind::UnknownEnvironment(env_name)));
                }
                TokenError::DisallowedChars => {
                    return Err(LatexError(loc, LatexErrKind::DisallowedChars));
                }
                TokenError::MissingBrace(ch) => {
                    return Err(LatexError(
                        loc,
                        LatexErrKind::UnexpectedToken {
        ...
        let c: char = match tokloc.token() {
            ...
            Token::Error(err) => match err {
                TokenError::UnknownCommand(name) => {
                    return Err(LatexError(
                        tokloc.location(),
                        LatexErrKind::UnknownCommand(name),
                    ));
                }
                TokenError::UnknownEnvironment(env_name) => {
                    return Err(LatexError(
                ...
                TokenError::DisallowedChars => {
                    return Err(LatexError(tokloc.location(), LatexErrKind::DisallowedChars));
                }
                TokenError::MissingBrace(ch) => {
                    return Err(LatexError(
                        tokloc.location(),
